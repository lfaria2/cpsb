...
\subsection{Board B}\index{Board!Board B}
   Board B is where the ROM chip containing all the assets and instructions specific to a game are attached via DIP sockets. The chips are not soldered but push-in mounted (and easily removable).
...
\pagebreak
   ...
   There are 3x8 = 24 chips, referred to as "GFX ROM", dedicated to storing GFX via sockets [1-8], sockets [10-17], and sockets [20-27] for a total of 12 MiB capacity. Because of the price of ROM, games were never budgeted to the max capacity allowed. Most titles were granted 2/4MiB, three (the Street Fighter II series) were allowed 6 MiB, and one (Dynasty Wars) got a whooping 8 MiB.
...
Could it have been an oversight? Could it actually have been a calculation error? In all likelihood the hardware designers wanted to give the CPS-1 a very high horizontal resolution to make it competitive, even if this meant making artists' lives a little bit difficult.
...
\subsection{ID check}
The simplest copy protection available is the chip ID check. By polling a register, the m68k prompts the CPS-B to return its version number. A version match lets the code know if it belongs to the right PCB and resets the CPU if it doesn't. 
...
is 8x8 tiles, 16x16 tiles, 16x16 sprites, 32x32 tiles, and all games tested only
